...
public record TpaMenuConfig(
  ...
  public static TpaMenuConfig defaults() {
    return new TpaMenuConfig(
        ...
        "dd/MM HH:mm",
        List.of(
            "{type} {target}.", "Result: {status}", "Where: {world} {x}, {y}, {z}", "When: {time}"),
        Material.BARRIER,
        "No history yet",
        List.of(
            "You have no recent teleports.",
            "",
            "/tpa <player> to visit someone.",
            "/tpahere <player> to summon someone."),
        "accepted",
        "denied",
        "expired",
        "cancelled",
        ...
        Material.HOPPER,
        "Filter: {filter}",
        List.of("Filters the shown results.", "", "{options}", "", "Click to cycle the filter."),
        "all",
        " ◀",
        45,
        Material.ARROW,
