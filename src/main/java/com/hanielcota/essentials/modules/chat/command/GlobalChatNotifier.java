...
/**
 * Owns the side-effects of {@code /g}: applies the same cooldown + anti-spam guard that the chat
 * listener uses, renders the message through {@link ChatFormatter} with the global template, and
 * broadcasts via {@link AudienceProvider#broadcast()}. The command path bypasses {@link
...
public final class GlobalChatNotifier {
  ...
  public void sendOneShot(@NonNull Player sender, @NonNull String body) {
    if (this.guard.shouldBlock(sender, this.channel, body)) {
      return;
    }

    var senderId = sender.getUniqueId();
    this.guard.touch(senderId, this.channel.id(), body);
